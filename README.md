...
- **99% of Seattle residents** live within a 10-minute walk of a park. Only **11.9%** live within a 10-minute network walk of an OLA (repo-computed April 2026; see Methodology).
...
- **11.9% walkshed (Part II, computed April 2026)** — the "residents within a 10-minute walk of an OLA" figure is produced by `scripts/compute_walkshed.py` (osmnx against Seattle's OpenStreetMap walk network, projected to UTM 10N, physical barriers respected) and `scripts/population_coverage.py` (2020 Census block-group overlay clipped to Seattle city boundary, area-weighted). Output at `data/walkshed/population_coverage.csv`. The isochrone uses convex hulls of reachable network nodes and may slightly overstate coverage at OLA boundaries; a TPL ParkServe-equivalent analysis could shift this by a percentage point or two, likely down. Supersedes the earlier ~33% straight-line estimate.
