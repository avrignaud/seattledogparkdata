...
Ahead of rebuilding `docs/enforcement.html` for the 2014–2026 window, every number destined for the page was independently reproduced from the **raw** PRR XLSX files (not the consolidated CSV) and cross-checked against both the page's data and its hardcoded prose. 77/78 data checks passed on the first pass; the one failure and three additional prose issues were corrected:

- **Cost per citation 2022:** prose said $1,716; correct value is **$1,730** ($292,399 / 169). Fixed.
- **First-offense floor:** stat card and prose said "85–96%"; the 2014 value is **83.6%**, so the range is now stated as "about 84–96%." Fixed.
- **Named-park share:** prose said "about 85%" (the old 2014–2019 figure); for the full dataset it is **88.7%**, now "about 89%." Fixed.
- **Pre-2016 imputation artifacts:** the imputed part-time cost/FTE made 2015 show the *lowest* cost-per-citation ($147) and *highest* per-FTE (1,038), contradicting the "$229 low / 638 peak in 2018" narrative. Resolved by starting the cost-per-citation and per-FTE charts at **2016** (the first MOA-documented year) while keeping raw volume bars across the full 2014–2026 span. The "638 peak in 2018" and "$229 low in 2018" claims are true within the shown 2016+ range.

**Cost-anchor verified from source PDF (May 2026).** The FAS ACO II annual cost constant `$152,399` was confirmed verbatim against `data/moas/SPR-FAS-ACO2-MOA-2021-AG21-PRF03-032.pdf`, Attachment A: hourly $43.07 × 1.45 OH = $62.45 × 2,088 hrs = $130,398.73 salary+benefits, plus $3,000 supplies, plus $19,000 divisional overhead = grand total **$152,398.73** (rounds to $152,399). The `$140,000` FMW figure remains an **author estimate** (SPR publishes no per-FMW off-leash line) and is the softest load-bearing number in the cost model — at FMW=$80K cumulative cost-recovery rises to ~13.5%, at $200K it falls to ~9% (vs. 10.5% reported at $140K). Flagged as estimate in the build script and methodology.

**2014 confirmed full calendar year (May 2026).** C049204's 2014 rows span 2014-01-06 → 2014-12-20 with citations in all twelve months. The low 183-citation count is genuine low-enforcement-era data (0.5 imputed ACO FTE, pre-MOA), not a partial-year or program-startup artifact.

New committed artifact: [`data/enforcement-year-metrics.csv`](data/enforcement-year-metrics.csv), built by `scripts/build_enforcement_metrics.py` and verified by `scripts/verify_enforcement_data.py` (recomputes every per-year metric from the consolidated CSV + documented cost model; cross-checks cumulative cost $3.30M, revenue $351,099, recovery ~11%). The production page (`docs/enforcement.html`) is generated by the committed `scripts/build_enforcement_page.py` from `scripts/enforcement_page_data.json`, itself derived from the consolidated CSV — so every page number is reproducible from committed scripts, none hand-typed. (The page was promoted from a review draft to live `enforcement.html` in the May 2026 publish; the draft file and its `noindex` banner were retired.)

**HTML prose check added (May 2026).** The verifier's CSV checks guarantee the *data* is correct but not that numbers typed into the page's prose match it. A new `verify_html_prose()` step (verifier step [11]) recomputes ~35 load-bearing prose figures from the data and asserts each formatted string appears in the rendered draft HTML — a grep-style guard against fat-fingered or stale prose numbers. It immediately caught two:

- **Top-10 post-COVID concentration:** prose said "33% after"; the data (top-10 named-park share, 2020–2026) is **40%**. Fixed. (Pre-COVID 46% was correct.)
- **"Best post-COVID year" superlative:** 2024's 447 was labeled the best post-COVID year, but 2021's 471 is higher and sits inside the chart's shaded 2020–2021 COVID band. Reworded to "the strongest year since the COVID period (2024)," which is unambiguous and keeps the 447/35%-of-peak arithmetic correct.
...
- "4,803 off-leash citations 2014–2019" → matched the original row count of enforcement-citations.csv prior to the May 2026 ingest of PRR C263949. After ingest, the CSV's 2019 rows are sourced from C263949 (full year, all-category) instead of C049204 (Jan-Oct, DLP only); the DLP-only 2014–2019 count is now 4,955 (3,774 from C049204 2014-2018 + 1,181 from C263949 2019). The legacy "4,803" claim has been **superseded**: the rebuilt enforcement.html (live May 2026) covers the full 2014–2026 record (7,015 DLP citations) and no longer cites 4,803; the underlying raw C049204 still has exactly 4,803 rows.
