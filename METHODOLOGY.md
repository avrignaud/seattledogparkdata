...
| Chart | Methodology notes |
|---|---|
| 10-minute walkshed coverage | 99% figure from [TPL 2025 ParkScore Seattle PDF](https://parkserve.tpl.org/downloads/pdfs/New%20York_NY.pdf) — formal network-based walkshed. 33% OLA figure is an **author estimate** using straight-line half-mile buffers around OLA coordinates from `seattle-olas.csv`, overlaid on 2020 Census block-group population. It does not account for physical barriers (I-5, Ship Canal) and likely overstates coverage; a formal TPL-style network walkshed would return a smaller number. See "Network walkshed" below for the planned upgrade. |
...
## Planned methodology upgrades (in priority order)

1. **Network walkshed in Python.** Replace the straight-line 33% OLA access estimate with a proper 0.5-mile walkshed computed using [osmnx](https://github.com/gboeing/osmnx) + Seattle's OpenStreetMap road network, with physical barriers respected (I-5, SR-99, Ship Canal). Output: a GeoJSON of walkshed polygons + a block-group-weighted coverage percentage. Directly comparable to TPL's method.
